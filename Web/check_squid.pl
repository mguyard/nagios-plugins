...
use vars qw($opt_V $opt_h $opt_t $opt_u $opt_n $opt_s
			$opt_p $opt_l $opt_o $opt_m $opt_e $opt_r $opt_d);
...
my ($url, $urluser, $urlpass, $proxy, $proxyport,
     $proxyuser, $proxypass, $expectstatus, $regex, $res, $req);
...
print $res->content if $opt_d;

if ($res->status_line =~ /^$expectstatus/)
{
	if ($regex) {
		if ( $res->content =~ m/$regex/) {
			print "OK - Status: ".$res->status_line." and find '$regex' in return\n";
			exit $ERRORS{"OK"};
		}else{
			print "CRITICAL - Status: ".$res->status_line." but don't find string '$regex' in return\n";
...
sub print_usage () {
	...
	print " <return http code> -r <regex> [-d]\n";
}

sub print_help () {
	print_revision($PROGNAME,'$Revision: 1 $');
	print "Perl check squid proxy\n";
	print "Modify by M. GUYARD to add regex check\n";
...
-r, --regex=STRING
	Search a string in the returned content
